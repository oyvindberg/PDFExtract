...
private static boolean tryHorizontalSplit(final PhysicalPageRegion r,
                                          final Rectangle dims,
                                          final float fractionToConsider,
                                          final int minimumDistanceToSplit)
{
    final float startY = dims.y;
    final float endY = Math.min(r.getPos().endY, startY + dims.height * fractionToConsider);
