                                                ...
                                                final float boundaryEndX)
{
    ...
    for (int y = (int) rpos.y; y <= (int) (rpos.endY + 1.0F); y++) {
        ...
        PhysicalContent closestOnRight = getClosestToTheRightAtY(everythingRightOf,
                                                                 y,
    ...
    final Rectangle adjusted = new Rectangle(boundaryStartX,
                                             realBoundaryY + 2.0F,
                                             1.0f,
                                             Math.max(0.1f,
                                                      realBoundaryEndY - realBoundaryY - 3.0F));
...
private static void filter(final PhysicalPageRegion r, final List<WhitespaceRectangle> boundaries) {
    ...
    for (int i = boundaries.size() - 1; i >= 0; i--) {
        ...
        Rectangle search = new Rectangle(boundaryToTheLeft, bpos.y, searchWidth, bpos.height);
...
private static List<PhysicalContent> findAllContentsRightOf(@NotNull PhysicalPageRegion region,
                                                            float x)
{
    final Rectangle search = new Rectangle(x,
                                           region.getPos().y,
                                           region.getPos().width,
                                                                         ...
                                                                         @NotNull List<WhitespaceRectangle> whitespaces)
{
    ...
    for (WhitespaceRectangle whitespace : whitespaces) {
        ...
        final List<PhysicalContent> left = region.searchInDirectionFromOrigin(W,
                                                                              smallerPos,
                                                                              LOOKAHEAD);
        ...
        for (PhysicalContent content : left) {
            ...
            if (MathUtils.isWithinVariance(content.getPos().endX,
                                           posX + HALF_LOOKAHEAD,
                                           LOOKAHEAD)) {
                leftCount++;
            }
        ...
        final List<PhysicalContent> right = region.searchInDirectionFromOrigin(E,
                                                                               smallerPos,
                                                                               LOOKAHEAD);
        ...
        for (PhysicalContent content : right) {
            ...
            if (MathUtils.isWithinVariance(content.getPos().x,
                                           posEndX + HALF_LOOKAHEAD,
                                           LOOKAHEAD)) {
                rightCount++;
            }
        }
