...
import org.elacin.pdfextract.physical.segmentation.graphics.PathSplitter;
...
public void drawImage(@NotNull final Image image,
    ...
    org.elacin.pdfextract.util.Rectangle pos;
    try {
        pos = new org.elacin.pdfextract.util.Rectangle(x, y, endX - x, endY - y);
    } catch (Exception e) {
...
public List<GraphicContent> getGraphicContents() {
    ...
    for (GeneralPath figurePath : figurePaths) {
        try {
            final org.elacin.pdfextract.util.Rectangle pos = convertRectangle(figurePath.getBounds());
